...
4. Confirm `DisplayMagicianConsole OperationStatus` succeeds and `DisplayMagicianConsole OperationDecisions` shows no pending decisions.
...
3. Run `DisplayMagicianConsole OperationStatus -p` while the shortcut is active. Confirm each line contains operation ID, sequence, phase, terminal/success flags, UTC timestamp, and message.
...
3. In a second command prompt run `DisplayMagicianConsole OperationDecisions -p`, copy the prompt ID, then run `DisplayMagicianConsole ResolveOperationDecision <Prompt_ID> Continue`.
...
6. Leave the prompt unanswered until expiry. Confirm it defaults to Continue and that a later `ResolveOperationDecision` command is rejected.
