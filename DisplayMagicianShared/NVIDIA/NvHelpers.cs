...
namespace DisplayMagicianShared.NVIDIA
{
                    ...
                    foreach (var field in type.GetRuntimeFields())
                    {
                        ...
                        else if (field.FieldType.IsArray)
                        {
                            ...
                            var array = Array.CreateInstance(
                                arrayType ?? throw new InvalidOperationException($"Field type {field.Name} is null when instantiating type {type.Name}."), size ?? 0);
    ...
    public struct ValueTypeArray : IDisposable, IHandle, IEquatable<ValueTypeArray>
    {
        ...
        public static ValueTypeArray FromList(IEnumerable<object> list)
        {
            var array = list.ToArray();
        ...
        // ReSharper disable once ExcessiveIndentation
        // ReSharper disable once MethodTooLong
        public static ValueTypeArray FromList(IEnumerable<object> list, Type type)
        {
            var array = list.ToArray();
        ...
        public static ValueTypeArray FromList<T>(List<T> list) where T : struct
        {
            return FromList(list.Cast<object>());
        }


        public List<T> ToList<T>(int count) where T : struct
        {
            return ToList<T>(count, typeof(T));
        }

        public List<T> ToList<T>(int count, Type type)
        {
            return ToList<T>(0, count, type);
        }

        public List<T> ToList<T>(int start, int count) where T : struct
        {
            return ToList<T>(start, count, typeof(T)).ToList();
        }

        public List<T> ToList<T>(int start, int count, Type type)
        {
            if (IsNull)
            {
                return null;
            ...
            return AsEnumerable<T>(start, count, type).ToList();
        }
    ...
    [StructLayout(LayoutKind.Sequential)]
    public struct ValueTypeArray<T> : IDisposable, IHandle, IEquatable<ValueTypeArray<T>> where T : struct
    {
        ...
        public static ValueTypeArray<T> FromList(List<T> list)
        {
            return new ValueTypeArray<T>(ValueTypeArray.FromList(list));
        }

        public static ValueTypeArray<T> FromList(IEnumerable<T> list, Type type)
        {
            return new ValueTypeArray<T>(ValueTypeArray.FromList(list.Cast<object>(), type));
        }
        ...
        {
            return underlyingArray.ToList<T>(count, typeof(T));
        }

        public List<T> ToList(int count, Type type)
        {
            return underlyingArray.ToList<T>(0, count, type);
        }

        public List<T> ToList(int start, int count)
        {
            return underlyingArray.ToList<T>(start, count, typeof(T)).ToList();
        }

        public List<T> ToList(int start, int count, Type type)
        {
            return underlyingArray.ToList<T>(start, count, type);
        }
    ...
    [StructLayout(LayoutKind.Sequential)]
    public struct ValueTypeReference<T> : IDisposable, IHandle, IEquatable<ValueTypeReference<T>> where T : struct
    {
