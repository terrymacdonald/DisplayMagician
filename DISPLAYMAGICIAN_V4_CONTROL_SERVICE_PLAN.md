...
### Phase A Ã¢â‚¬â€ Structure and IPC
...
### Phase B Ã¢â‚¬â€ Storage and migration
...
### Phase C Ã¢â‚¬â€ First end-to-end profile operation
...
### Phase D Ã¢â‚¬â€ Shortcut lifecycle and recovery
...
### Phase E Ã¢â‚¬â€ Security, background ownership, and diagnostics
...
### Phase E Part 2 Ã¢â‚¬â€ Unified support logging and timeline contract
...
- [x] Treat these as the only support-log component names: `DesktopApp`, `DesktopConsole`, `UserAgent`, `ControlService`, `SessionLauncher`, and `Installer`.
...
- [x] Keep component identity at process/host level. Use `source` and, where useful later, a bounded subsystem field for features such as display, audio, shortcuts, migration, or update handling.
...
- [x] Retain the existing `ClassName/MethodName:` convention as the authoritative `source` value. Do not depend on automatic call-site capture as the source of truth, particularly across async code.
...
#### NLog source migration Ã¢â‚¬â€ all C# files
...
- [x] Refactor every retained diagnostic NLog call in those component C# files to the common logging/correlation path. It must render the required fields and explicit `source`, correctly serialize C# exceptions, and carry current request/operation IDs where available.
- [x] Preserve `DesktopConsole` command results, help, and machine-parseable stdout/stderr behaviour as its public CLI contract. Only its diagnostics are migrated to the log contract; do not pollute normal command output with logfmt events.
- [x] Remove legacy message decorations that duplicate structured fields, such as `ERROR -`, ad-hoc timestamps, or inconsistent component prefixes. Preserve diagnostically useful human text in `msg` and retain the existing `ClassName/MethodName` source identity.
- [x] Replace direct component-level NLog configuration with the shared contract configuration, while allowing only the approved bootstrap/configuration files to create targets or obtain raw NLog loggers.
- [x] Provide a lightweight human-run source audit, `verify_log_message_formatting.ps1`, for direct NLog source prefixes. Do not make it a per-build blocker.
- [ ] Run `verify_log_message_formatting.ps1` as a release/pre-release check whenever logging call sites change. It remains a human-run root script rather than a per-build blocker.
...
- [x] Preserve the existing `OperationId` GUID as the ID for one meaningful multi-step user action, such as a shortcut run. Generate it once at the action boundary and pass it through status updates, Service routing, Agent work, recovery, and related logs.
- [x] Add correlation context that flows correctly through asynchronous work and explicitly crosses the DesktopApp/Console -> ControlService -> SessionLauncher -> UserAgent boundaries. Do not generate a replacement operation ID downstream.
- [x] Ensure background, startup, and unrelated lifecycle events render `operation_id=-` and `request_id=-` rather than inheriting stale context.
...
- [x] Define an Installer log policy: installation, repair, upgrade, and uninstall must be able to produce a verbose MSI log with a known support-collectable location or user-selected export path. Do not claim installer logs are available when installation was run without logging enabled.
...
- [x] Maintain backward compatibility for retained legacy log files: include them where safe, identify their format in the manifest, and do not require the timeline importer to guess that they follow the new contract.
...
- [x] Define its importer to parse one log line into table cells: time, component, level, source/function, operation ID, request ID, message, and optional exception details.
- [x] Require fast multi-select intersection filtering by time range, component, level, source, operation ID, request ID, and free-text message/exception search. Show exception details and the original raw line in an expandable event-details view rather than default table columns.
- [x] Merge parsed events by UTC timestamp, retain a deterministic tie-breaker (bundle path plus line number), and clearly flag malformed or legacy lines without discarding the rest of a bundle.
- [x] Use manifest schema/version and log inventory to select parsers; support future schema additions without breaking older Support ZIP imports.
...
- [x] Update the user-facing **Create a Support ZIP File** workflow after the new component log targets exist. It must collect every retained log file for `DesktopApp`, `DesktopConsole`, `UserAgent`, `ControlService`, and `SessionLauncher`; do not rely on a shared directory name to imply that a component is covered.
- [x] Have the Control Service stage both machine-owned sources (`ControlService` and `SessionLauncher`) for the current authorized user, then remove the staging copy after the User Agent has written the ZIP. The desktop client must never read protected machine log paths directly.
- [x] Include every approved configuration source in a clearly named `Configuration/` area: profiles, audio profiles, shortcuts, settings, migration state, legacy/recovery configuration, and relevant machine service state. Retain the agreed exclusions for credentials, tokens, unrelated user content, media, wallpapers, and cached icons.
- [x] Add each included component log and configuration file to `support-manifest.json`, including component name, ZIP entry path, source format/version, collection result, and a warning for every unavailable source. Do not silently omit a requested component or configuration area.
...
### Phase E Part 3 Ã¢â‚¬â€ Shared operation status and user decisions
...
- [x] Define an operation origin and notification preference for future clients without trusting unverified client-supplied identity. The Control Service derives the authoritative origin from the authenticated transport and Windows user/session; an unverified client label is display-only metadata. Phase G will persist notification preferences per paired device, while v4 broadcasts status and decisions to the authenticated same-user/session audience by default.
...
- [ ] Unit-test ordered status publication, operation correlation, default Continue on expiry, first-valid-response-wins, duplicate/late rejection, and display/audio/start-program/stop-program recovery or terminal transitions. Store-level ordering, ownership, first-response, expiry-default, and pending-decision recovery coverage is complete; hardware-dependent transitions remain Sandbox/manual checks.
...
### Phase F Ã¢â‚¬â€ Deployment hardening
...
### Phase G Ã¢â‚¬â€ Local REST foundation
...
### Phase H Ã¢â‚¬â€ Final v4.0.0 work Ã¢â‚¬â€ Steam Big Picture
...
### Phase I Ã¢â‚¬â€ Final verification and release readiness
