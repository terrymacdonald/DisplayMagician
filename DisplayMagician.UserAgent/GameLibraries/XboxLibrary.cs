...
namespace DisplayMagician.GameLibraries
{
    public sealed class XboxLibrary : GameLibrary
    {
        ...
        public override bool IsGameLibraryInstalled
        {
            get
            {
                return HasPackageIdentity && _isXboxInstalled;
            }
        ...
        private static string NormaliseGameName(string name)
        {
            ...
            string normalisedName = Regex.Replace(name, @"[\u2122\u00a9\u00ae]", string.Empty)
                .Replace("_", " ")
                .Replace(".", " ")
                .Replace('\u2019', '\'');
            string withoutBrackets = Regex.Replace(normalisedName, @"\[.*?\]", string.Empty);
            normalisedName = string.IsNullOrWhiteSpace(withoutBrackets) ? normalisedName : withoutBrackets;
            string withoutParentheses = Regex.Replace(normalisedName, @"\(.*?\)", string.Empty);
            normalisedName = string.IsNullOrWhiteSpace(withoutParentheses) ? normalisedName : withoutParentheses;
            normalisedName = Regex.Replace(normalisedName, @"\s*:\s*", ": ");
            normalisedName = Regex.Replace(normalisedName, @"\s+", " ");
            if (Regex.IsMatch(normalisedName, @",\s*The$", RegexOptions.IgnoreCase))
            {
                normalisedName = "The " + Regex.Replace(normalisedName, @",\s*The$", string.Empty, RegexOptions.IgnoreCase);
            }
