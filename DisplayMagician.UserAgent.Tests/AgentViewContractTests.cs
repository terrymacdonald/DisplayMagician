...
public sealed class AgentViewContractTests
{
    ...
    public void ProfileListResult_RoundTripsCurrentLayoutAndProfileSettings()
    {
        ...
        Assert.Single(roundTripped!.Views);
        Assert.Equal("saved-profile", roundTripped.Views[0].Id);
        Assert.Equal(2, roundTripped.Views[0].ConnectedDisplayCount);
        Assert.Equal(3840, roundTripped.Views[0].PrimaryDisplayWidth);
        Assert.True(roundTripped.Views[0].Settings.ApplyWallpaper);
        Assert.Equal("Span across displays", roundTripped.Views[0].Settings.BackgroundDescription);
        Assert.True(roundTripped.Views[0].Settings.ForceExplorerRestart);
        Assert.Equal(3, roundTripped.Views[0].Settings.ApplyProfileCount);
        Assert.Equal(750, roundTripped.Views[0].Settings.ApplyProfileDelay);
