...
- Trust boundary: a caller on the same Windows device in the same signed-in Windows user context may use the local named-pipe API and is treated as a trusted local caller. A caller on another device or in another Windows user context must use the remote REST gateway and is untrusted until it has authenticated, paired, been authorised for the requested capability, and passed expiry and replay protection. Never expose ControlService directly to the network.
- Local integrations such as a Stream Deck plugin use the same-user named-pipe path, normally through a narrow local bridge where their runtime cannot use named pipes directly. Remote integrations use the paired-device REST path; do not silently fall back from a remote target to local-pipe trust.
...
- Every transport request carries the shared `ProtocolHello` compatibility metadata and a successful response carries `ProtocolWelcome`. Validate the negotiated version and required capabilities before performing work; use these same contracts for local pipes now and REST later. Compatibility negotiation is not authentication, pairing, or licence validation.
